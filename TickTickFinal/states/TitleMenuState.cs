...
class TitleMenuState : GameObjectList
{
    ...
    public TitleMenuState()
    {
        ...
        SpriteGameObject titleScreen = new SpriteGameObject("Backgrounds/spr_title", Camera.UILayer, "background");
        Add(titleScreen);

        // add a play button
        playButton = new Button("Sprites/spr_button_play", Camera.UILayer + 1);
        playButton.Position = new Vector2((GameEnvironment.Screen.X - playButton.Width) / 2, 540);
        Add(playButton);

        // add a help button
        helpButton = new Button("Sprites/spr_button_help", Camera.UILayer + 1);
